...
public class UserService {
    ...
    private static final java.math.BigDecimal BMI_UNDERWEIGHT =
            new java.math.BigDecimal("18.5");

    private static final java.math.BigDecimal BMI_NORMAL_MAX =
            new java.math.BigDecimal("24.9");

    private static final java.math.BigDecimal BMI_OVERWEIGHT_MAX =
            new java.math.BigDecimal("29.9");

    private static final java.math.BigDecimal MIN_WEIGHT_KG =
            new java.math.BigDecimal("1");

    private static final java.math.BigDecimal MAX_WEIGHT_KG =
            new java.math.BigDecimal("500");

    private static final java.math.BigDecimal MIN_HEIGHT_CM =
            new java.math.BigDecimal("30");

    private static final java.math.BigDecimal MAX_HEIGHT_CM =
            new java.math.BigDecimal("300");

    @Transactional
    public HealthProfileResponseDTO actualizarPerfilDeSalud(
            Long userId,
        ...
        if (dto.getWeightKg() != null &&
                (dto.getWeightKg().compareTo(MIN_WEIGHT_KG) < 0 ||
                        dto.getWeightKg().compareTo(MAX_WEIGHT_KG) > 0)) {

            throw new ResponseStatusException(
                    HttpStatus.BAD_REQUEST,
                    "Peso inválido: " + dto.getWeightKg() + " kg. Rango: 1-500 kg."
            );
        }

        if (dto.getHeightCm() != null &&
                (dto.getHeightCm().compareTo(MIN_HEIGHT_CM) < 0 ||
                        dto.getHeightCm().compareTo(MAX_HEIGHT_CM) > 0)) {

            throw new ResponseStatusException(
                    HttpStatus.BAD_REQUEST,
                    "Altura inválida: " + dto.getHeightCm() + " cm. Rango: 30-300 cm."
            );
        }

        HealthProfile profile =
                user.getHealthProfile() != null
        ...
        if (profile.getWeightKg() != null &&
            ...
            java.math.BigDecimal hM =
                    profile.getHeightCm().divide(
            ...
            java.math.BigDecimal bmi =
                    profile.getWeightKg().divide(
                            hM.pow(2, new java.math.MathContext(6)),
                            2,
                            java.math.RoundingMode.HALF_UP
                    );
            ...
            profile.setBmiCategory(
                    bmi.compareTo(BMI_UNDERWEIGHT) < 0
                            ? "Bajo peso"
                            : bmi.compareTo(BMI_NORMAL_MAX) <= 0
                            ? "Normal"
                            : bmi.compareTo(BMI_OVERWEIGHT_MAX) <= 0
                            ? "Sobrepeso"
                            : "Obesidad"
            );
        }

        return toHealthProfileDto(
                healthProfileRepository.save(profile)
        );
    }

    // ── HU07 — Signos vitales + Alertas ───────────────────────────────

    private static final java.math.BigDecimal GLUCOSE_ALERT_THRESHOLD =
            new java.math.BigDecimal("126");

    private static final java.math.BigDecimal CHOLESTEROL_ALERT_THRESHOLD =
            new java.math.BigDecimal("240");

    private static final int SYSTOLIC_ALERT_THRESHOLD = 140;
    private static final int DIASTOLIC_ALERT_THRESHOLD = 90;

    @Transactional
    public HealthProfileResponseDTO registrarSignosVitales(
            Long userId,
        ...
        if (dto.getGlucoseLevel() != null) {
            ...
            if (dto.getGlucoseLevel().compareTo(GLUCOSE_ALERT_THRESHOLD) >= 0) {
                ...
                alert.append("ALERTA: Glucosa ")
                        .append(dto.getGlucoseLevel())
                        .append(" mg/dL >= 126 (ADA). ");
            }
        }

        if (dto.getBloodPressure() != null &&
                !dto.getBloodPressure().isBlank()) {

            profile.setBloodPressure(dto.getBloodPressure());

            String[] p = dto.getBloodPressure()
                    .replace("mmHg", "")
                    .strip()
                    .split("/");

            int sys = Integer.parseInt(p[0].strip());
            int dia = Integer.parseInt(p[1].strip());

            if (sys >= SYSTOLIC_ALERT_THRESHOLD ||
                    dia >= DIASTOLIC_ALERT_THRESHOLD) {

                critical = true;

                alert.append("ALERTA: Presión ")
        ...
        if (dto.getCholesterolLevel() != null) {
            ...
            if (dto.getCholesterolLevel()
                    .compareTo(CHOLESTEROL_ALERT_THRESHOLD) >= 0) {

                critical = true;

                alert.append("ALERTA: Colesterol ")
                        .append(dto.getCholesterolLevel())
                        .append(" mg/dL >= 240 (NCEP). ");
            }
        }
        ...
        profile.setAlertMessage(
                critical
                        ? alert.toString().strip()
                        : null
        );
